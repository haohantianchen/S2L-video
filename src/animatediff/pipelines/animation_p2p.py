...
#            for i, t in enumerate(timesteps):
            while i < len(timesteps):
                ...
                prior_overlap = 0
                for context, my_overlap in context_scheduler(
                    i, num_inference_steps, latents.shape[2], context_frames, context_stride, context_overlap, False
                ):
                    ...
                    if count >= 2:
                        print("keep prior block: ",context[:prior_overlap])
                        print("update last block: ", context[prior_overlap:])
                        latents[:, :, context[:prior_overlap]] = ori_latents[:, :, context[:prior_overlap]]
                        if up_flag:
                            zero_idx = context.index(0)
                            print("restore block: ",context[zero_idx:])
                            latents[:, :, context[zero_idx:]] = ori_latents[:, :, context[zero_idx:]]

                    
                    # 判断最后一个block update的帧数是否loop
                    if latents.shape[2] - 1 - context[-1] < 16 - my_overlap:
                        up_flag = True


                    prior_overlap = my_overlap
