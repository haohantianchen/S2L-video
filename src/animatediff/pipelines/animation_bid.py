...
class MybidAnimationPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
...
#                self.vae.decode(latents[frame_idx : frame_idx + 1].to(self.vae.device, self.vae.dtype)).sample.cpu()
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            # noise_rand = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            noise_rand = torch.randn(shape, generator=generator, device="cpu", dtype=dtype, layout=torch.strided).to(device)
            # noise_base = randn_tensor(shape_base, generator=generator, device=device, dtype=dtype)
            noise_base = torch.randn(shape_base, generator=generator, device="cpu", dtype=dtype, layout=torch.strided).to(device)
            noise_repeat_re = noise_base.repeat(1, 1, math.ceil(video_length / 16), 1, 1)
            if noise_repeat_re.shape[2] == noise_rand.shape[2]:
                noise_repeat = noise_repeat_re
            else:
                noise_repeat = noise_repeat_re[:,:,:noise_rand.shape[2]]
            m = 0.5
            noise = (1 - m) * noise_rand + m * noise_repeat

            # 标准化 noise 以符合 torch.randn 的分布
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 新增准备latents input
        # latents_base = latents[:,:,:context_overlap]
        # m = torch.randn(latents.shape[0], latents.shape[1], 1, 1, 1).to(latents.dtype)
        # latents_repeated = latents_base.repeat(1, 1, latents.shape[2] // context_overlap, 1, 1)
        # latents = (1 - m) * latents + m * latents_repeated


        latents, noise, image_latents = latents_outputs

        # latents_repeated = latents_base.repeat(1, 1, video_length // context_overlap, 1, 1)
        # rand_noise = randn_tensor(latents_repeated.shape, generator=generator, device=latents_base.device, dtype=latents_base.dtype)
        # m = 0.5
        # latents = (1 - m) * rand_noise + m * latents_repeated
...
#            for i, t in enumerate(timesteps):
            while i < len(timesteps):
                ...
                count = 0
                if i % 2 == 1:
                     context_list = list(context_scheduler(i-1, num_inference_steps, latents.shape[2], context_frames, context_stride, context_overlap))
                     context_list = context_list[::-1]
                else:
                    context_list = list(context_scheduler(i, num_inference_steps, latents.shape[2], context_frames, context_stride, context_overlap))
                print(context_list)
                ...
                for context in context_list:
                    ...
                    if do_classifier_free_guidance:
                        noise_list = list(noise_pred[:, :, context].chunk( noise_size ))
                    ...
                    latents[:, :, context] = self.scheduler.step(
                        model_output=noise_pred,
                        timestep=t,
                        sample=latents[:, :, context],
                        **extra_step_kwargs,
                        return_dict=False,
                    )[0]


                    # if 1< count <= (len(context_list) // 2):
                    if count > 1 and i % 2 == 0:
                        print(context[:context_overlap])
                        # latents[:, :, context[:context_overlap]] = 0.5 * ori_latents[:, :, context[:context_overlap]] + 0.5 * latents[:, :, context[:context_overlap]]
                        latents[:, :, context[:context_overlap]] = ori_latents[:, :, context[:context_overlap]]
                    if count > 1 and i % 2 == 1:
                        print(context[-context_overlap:])
                        # latents[:, :, context[:context_overlap]] = 0.5 * ori_latents[:, :, context[:context_overlap]] + 0.5 * latents[:, :, context[:context_overlap]]
                        latents[:, :, context[-context_overlap:]] = ori_latents[:, :, context[-context_overlap:]]
                    # elif count > (len(context_list) // 2) + 1:
                    #     print(context[-context_overlap:])
                    #     latents[:, :, context[-context_overlap:]] = ori_latents[:, :, context[-context_overlap:]]
